...
import {
  ...
  Trash2, Edit2, Send, AlertTriangle,
} from "lucide-react";
...
const COLORS = {
  bg: "#14181B", surface: "#1C2226", surfaceRaised: "#242B30", border: "#2E363B",
  textPrimary: "#EDEFF0", textSecondary: "#8B96A0", accent: "#FF6A2E",
  accentBlue: "#4A90C2", success: "#4CAF6D", danger: "#D8564A",
};

const seedClientes = [
  { id: "c1", nombre: "Marco Jiménez", telefono: "8888-1234", correo: "marco.j@gmail.com" },
  { id: "c2", nombre: "Diana Salas", telefono: "8777-5566", correo: "diana.salas@gmail.com" },
];
const seedVehiculos = [
  { id: "v1", clienteId: "c1", placa: "CAB123", marca: "Toyota", modelo: "Hilux", anio: "2019", km: "82,400" },
  { id: "v2", clienteId: "c2", placa: "BPZ045", marca: "Hyundai", modelo: "Tucson", anio: "2021", km: "31,200" },
];
const seedCitas = [
  { id: "a1", clienteId: "c1", vehiculoId: "v1", fecha: "2026-08-03", hora: "09:00", servicio: "Cambio de aceite", estado: "confirmada" },
  { id: "a2", clienteId: "c2", vehiculoId: "v2", fecha: "2026-08-03", hora: "11:30", servicio: "Revisión de frenos", estado: "pendiente" },
];
const seedServicios = [
  { id: "s1", nombre: "Cambio de aceite", precio: 18000 },
  { id: "s2", nombre: "Revisión de frenos", precio: 22000 },
];
const seedProveedores = [
  { id: "p1", nombre: "Repuestos del Valle", contacto: "Luis Vargas", telefono: "2222-3344", suministra: "Frenos, filtros" },
];
const seedRepuestos = [
  { id: "r1", nombre: "Filtro de aceite", stock: 12, precioCompra: 3500, precioVenta: 6000, proveedorId: "p1" },
  { id: "r2", nombre: "Pastillas de freno", stock: 3, precioCompra: 15000, precioVenta: 24000, proveedorId: "p1" },
];
const seedOrdenes = [
  { id: "o1", vehiculoId: "v1", clienteId: "c1", fecha: "2026-07-28", estado: "completada", manoObra: 10000,
    items: [{ tipo: "servicio", refId: "s1", nombre: "Cambio de aceite", precio: 18000, cantidad: 1 }, { tipo: "repuesto", refId: "r1", nombre: "Filtro de aceite", precio: 6000, cantidad: 1 }],
    costoTotal: 34000 },
];
const seedFacturas = [
  { id: "f1", ordenId: "o1", clienteId: "c1", monto: 34000, estadoEnvio: "enviada", fecha: "2026-07-28" },
];

function TicketBadge({ n }) {
  return <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, color: COLORS.textSecondary, letterSpacing: "0.05em" }}>#{String(n).padStart(4, "0")}</div>;
}
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(10,12,14,0.65)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 20 }}>
      <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 10, width: wide ? 560 : 420, maxWidth: "100%", padding: 24, maxHeight: "85vh", overflow: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          ...
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: COLORS.textSecondary }}><X size={18} /></button>
        </div>
        {children}
      </div>
    </div>
...
const inputStyle = { width: "100%", boxSizing: "border-box", background: "#0e1114", border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: "9px 11px", color: COLORS.textPrimary, fontSize: 14, fontFamily: "'Inter', sans-serif", marginBottom: 14, outline: "none" };
const btnPrimary = { background: COLORS.accent, color: "#1C0D04", border: "none", borderRadius: 6, padding: "10px 16px", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif", width: "100%" };
const btnGhost = { background: "none", border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: 6, cursor: "pointer", color: COLORS.textSecondary };
...
export default function TallerApp() {
  ...
  function saveCliente() {
    ...
    if (editingCliente) setClientes(clientes.map((c) => (c.id === editingCliente ? { ...c, ...clienteForm } : c)));
    else setClientes([...clientes, { id: "c" + Date.now(), ...clienteForm }]);
    setShowClienteModal(false);
  }
  function deleteCliente(id) { setClientes(clientes.filter((c) => c.id !== id)); setVehiculos(vehiculos.filter((v) => v.clienteId !== id)); }
  ...
  function saveVehiculo() {
    ...
    if (editingVehiculo) setVehiculos(vehiculos.map((v) => (v.id === editingVehiculo ? { ...v, ...vehiculoForm } : v)));
    else setVehiculos([...vehiculos, { id: "v" + Date.now(), ...vehiculoForm }]);
    setShowVehiculoModal(false);
  }
  function deleteVehiculo(id) { setVehiculos(vehiculos.filter((v) => v.id !== id)); }
  ...
  function saveCita() {
    ...
    if (editingCita) setCitas(citas.map((c) => (c.id === editingCita ? { ...c, ...citaForm } : c)));
    else setCitas([...citas, { id: "a" + Date.now(), ...citaForm }]);
    setShowCitaModal(false);
  }
  function deleteCita(id) { setCitas(citas.filter((c) => c.id !== id)); }
  function toggleEstadoCita(c) {
    const siguiente = c.estado === "confirmada" ? "completada" : c.estado === "completada" ? "pendiente" : "confirmada";
    setCitas(citas.map((x) => (x.id === c.id ? { ...x, estado: siguiente } : x)));
  }
  ...
  function saveProveedor() {
    ...
    if (editingProveedor) setProveedores(proveedores.map((p) => (p.id === editingProveedor ? { ...p, ...proveedorForm } : p)));
    else setProveedores([...proveedores, { id: "p" + Date.now(), ...proveedorForm }]);
    setShowProveedorModal(false);
  }
  function deleteProveedor(id) { setProveedores(proveedores.filter((p) => p.id !== id)); }
  ...
  function saveRepuesto() {
    ...
    if (editingRepuesto) setRepuestos(repuestos.map((r) => (r.id === editingRepuesto ? { ...r, ...payload } : r)));
    else setRepuestos([...repuestos, { id: "r" + Date.now(), ...payload }]);
    setShowRepuestoModal(false);
  }
  function deleteRepuesto(id) { setRepuestos(repuestos.filter((r) => r.id !== id)); }

  const [nuevoServicioNombre, setNuevoServicioNombre] = useState("");
  const [nuevoServicioPrecio, setNuevoServicioPrecio] = useState("");
  function agregarServicio() {
    if (!nuevoServicioNombre.trim()) return;
    setServicios([...servicios, { id: "s" + Date.now(), nombre: nuevoServicioNombre, precio: Number(nuevoServicioPrecio) || 0 }]);
    setNuevoServicioNombre(""); setNuevoServicioPrecio("");
  }
  function eliminarServicio(id) { setServicios(servicios.filter((s) => s.id !== id)); }
  ...
  const costoOrdenActual = useMemo(() => ordenForm.items.reduce((sum, it) => sum + it.precio * it.cantidad, 0) + (Number(ordenForm.manoObra) || 0), [ordenForm.items, ordenForm.manoObra]);

  function openNewOrden() { setEditingOrden(null); setOrdenForm({ vehiculoId: "", items: [], manoObra: "", estado: "abierta", fecha: new Date().toISOString().slice(0, 10) }); setShowOrdenModal(true); }
  function openEditOrden(o) { setEditingOrden(o.id); setOrdenForm({ vehiculoId: o.vehiculoId, items: o.items || [], manoObra: o.manoObra || "", estado: o.estado || "abierta", fecha: o.fecha || "" }); setShowOrdenModal(true); }
  ...
  function quitarItemOrden(idx) { setOrdenForm({ ...ordenForm, items: ordenForm.items.filter((_, i) => i !== idx) }); }
  function saveOrden() {
    ...
    if (editingOrden) setOrdenes(ordenes.map((o) => (o.id === editingOrden ? { ...o, ...payload } : o)));
    else {
      setOrdenes([...ordenes, { id: "o" + Date.now(), ...payload }]);
      setRepuestos(repuestos.map((r) => {
        const usado = ordenForm.items.find((it) => it.tipo === "repuesto" && it.refId === r.id);
        return usado ? { ...r, stock: Math.max(0, (Number(r.stock) || 0) - usado.cantidad) } : r;
      }));
    }
    setShowOrdenModal(false);
  }
  function deleteOrden(id) { setOrdenes(ordenes.filter((o) => o.id !== id)); }
  function cambiarEstadoOrden(o, estado) { setOrdenes(ordenes.map((x) => (x.id === o.id ? { ...x, estado } : x))); }
  ...
  function openNewRevision() { setRevisionForm({ ordenId: "", tipo: "entrada", km: "", notas: "", checklist: {} }); setShowRevisionModal(true); }
  function toggleChecklistItem(item) { setRevisionForm({ ...revisionForm, checklist: { ...revisionForm.checklist, [item]: !revisionForm.checklist[item] } }); }
  function saveRevision() {
    if (!revisionForm.ordenId) return;
    setRevisiones([...revisiones, { id: "rv" + Date.now(), ...revisionForm }]);
    setShowRevisionModal(false);
  }
  function deleteRevision(id) { setRevisiones(revisiones.filter((r) => r.id !== id)); }
  ...
  function saveFactura() {
    ...
    setFacturas([...facturas, { id: "f" + Date.now(), ...facturaForm, clienteId: o?.clienteId || "", fecha: new Date().toISOString().slice(0, 10) }]);
    setShowFacturaModal(false);
  }
  function deleteFactura(id) { setFacturas(facturas.filter((f) => f.id !== id)); }
  function marcarEnviada(f) { setFacturas(facturas.map((x) => (x.id === f.id ? { ...x, estadoEnvio: "enviada" } : x))); }
  ...
  const stats = useMemo(() => {
    ...
    ordenes.forEach((o) => (o.items || []).forEach((it) => { if (it.tipo === "servicio") conteoServicios[it.nombre] = (conteoServicios[it.nombre] || 0) + 1; }));
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: COLORS.bg, minHeight: 640, display: "flex", color: COLORS.textPrimary, borderRadius: 12, overflow: "hidden", border: `1px solid ${COLORS.border}` }}>
      ...
      <div style={{ width: 216, background: "#191F23", borderRight: `1px solid ${COLORS.border}`, padding: "20px 12px", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "0 8px 22px", borderBottom: `1px solid ${COLORS.border}`, marginBottom: 14 }}>
          <div style={{ width: 30, height: 30, borderRadius: 7, background: COLORS.accent, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Wrench size={16} color="#1C0D04" /></div>
          <div>
        ...
        {NAV.map((item) => {
          ...
          return (
            <div key={item.id} onClick={() => setView(item.id)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 10px", borderRadius: 6, cursor: "pointer", marginBottom: 2, fontSize: 13.5, background: active ? COLORS.surfaceRaised : "transparent", color: active ? COLORS.textPrimary : "#9AA3A8", borderLeft: active ? `2px solid ${COLORS.accent}` : "2px solid transparent" }}>
      ...
      <div style={{ flex: 1, padding: "22px 28px", overflow: "auto" }}>
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
              ...
              <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 9, padding: 16 }}>
                ...
                <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 22, fontWeight: 600, color: repuestos.some((r) => Number(r.stock) < 5) ? COLORS.danger : COLORS.textPrimary }}>{repuestos.filter((r) => Number(r.stock) < 5).length}</div>
              </div>
            </div>
          </div>
        )}
        ...
        {view === "clientes" && (
          <div>
            <PageHeader title="Clientes" subtitle={`${clientes.length} clientes registrados`} action={<button onClick={openNewCliente} style={{ ...btnPrimary, width: "auto", display: "flex", alignItems: "center", gap: 6 }}><Plus size={15} /> Nuevo cliente</button>} />
              ...
              {filteredClientes.map((c, i) => (
                <div key={c.id} style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 9, padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    <div style={{ width: 38, height: 38, borderRadius: 8, background: COLORS.surfaceRaised, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Space Grotesk', sans-serif", fontWeight: 600, fontSize: 13.5, color: COLORS.accent }}>{c.nombre.split(" ").map((w) => w[0]).slice(0, 2).join("")}</div>
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: "#5D6870", marginTop: 4 }}>{vehiculosByCliente(c.id).length} vehículo{vehiculosByCliente(c.id).length !== 1 ? "s" : ""}{vehiculosByCliente(c.id).map((v) => ` · ${v.placa}`).join("")}</div>
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                    <TicketBadge n={i + 1} />
                    <button onClick={() => openNewVehiculo(c.id)} title="Agregar vehículo" style={{ ...btnGhost, marginLeft: 8 }}><Car size={14} /></button>
                    <button onClick={() => openEditCliente(c)} style={btnGhost}><Edit2 size={14} /></button>
                    <button onClick={() => deleteCliente(c.id)} style={{ ...btnGhost, color: COLORS.danger }}><Trash2 size={14} /></button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {view === "vehiculos" && (
          <div>
            <PageHeader title="Vehículos" subtitle={`${vehiculos.length} vehículos registrados`} action={<button onClick={() => openNewVehiculo(null)} style={{ ...btnPrimary, width: "auto", display: "flex", alignItems: "center", gap: 6 }}><Plus size={15} /> Nuevo vehículo</button>} />
        ...
        {view === "agenda" && (
          <div>
            <PageHeader title="Agenda" subtitle={`${citas.length} citas registradas`} action={<button onClick={openNewCita} style={{ ...btnPrimary, width: "auto", display: "flex", alignItems: "center", gap: 6 }}><Plus size={15} /> Nueva cita</button>} />
        ...
        {view === "ordenes" && (
          <div>
            <PageHeader title="Órdenes de trabajo" subtitle={`${ordenes.length} órdenes registradas`} action={<button onClick={openNewOrden} style={{ ...btnPrimary, width: "auto", display: "flex", alignItems: "center", gap: 6 }}><Plus size={15} /> Nueva orden</button>} />
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
                {servicios.map((s) => (
                  <span key={s.id} style={{ fontSize: 12, background: COLORS.surfaceRaised, borderRadius: 20, padding: "5px 10px", display: "flex", alignItems: "center", gap: 6 }}>
                    {s.nombre} · {money(s.precio)}<X size={11} style={{ cursor: "pointer", color: COLORS.textSecondary }} onClick={() => eliminarServicio(s.id)} />
                  </span>
                ))}
              </div>
                  ...
                  <div key={o.id} style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 9, padding: "14px 16px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      ...
                      <div style={{ textAlign: "right" }}>
                        ...
                        <select value={o.estado} onChange={(e) => cambiarEstadoOrden(o, e.target.value)} style={{ marginTop: 6, fontSize: 11.5, background: "transparent", color: estadoColor, border: `1px solid ${estadoColor}`, borderRadius: 20, padding: "3px 8px" }}>
                          <option value="abierta">Abierta</option><option value="en_progreso">En progreso</option><option value="completada">Completada</option>
                        </select>
                      </div>
                    </div>
        ...
        {view === "revisiones" && (
          <div>
            <PageHeader title="Revisión de entrada y salida" subtitle={`${revisiones.length} revisiones registradas`} action={<button onClick={openNewRevision} style={{ ...btnPrimary, width: "auto", display: "flex", alignItems: "center", gap: 6 }}><Plus size={15} /> Nueva revisión</button>} />
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {revisiones.map((r) => {
        ...
        {view === "inventario" && (
          <div>
            <PageHeader title="Inventario de repuestos" subtitle={`${repuestos.length} repuestos registrados`} action={<button onClick={openNewRepuesto} style={{ ...btnPrimary, width: "auto", display: "flex", alignItems: "center", gap: 6 }}><Plus size={15} /> Nuevo repuesto</button>} />
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}>
              {repuestos.map((r) => {
                ...
                return (
                  <div key={r.id} style={{ background: COLORS.surface, border: `1px solid ${bajoStock ? COLORS.danger : COLORS.border}`, borderRadius: 9, padding: 16 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      ...
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 18, fontWeight: 600, color: bajoStock ? COLORS.danger : COLORS.textPrimary, display: "flex", alignItems: "center", gap: 5 }}>{bajoStock && <AlertTriangle size={14} />} {r.stock}</div>
        ...
        {view === "proveedores" && (
          <div>
            <PageHeader title="Proveedores" subtitle={`${proveedores.length} proveedores registrados`} action={<button onClick={openNewProveedor} style={{ ...btnPrimary, width: "auto", display: "flex", alignItems: "center", gap: 6 }}><Plus size={15} /> Nuevo proveedor</button>} />
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {proveedores.map((p) => (
        ...
        {view === "facturacion" && (
          <div>
            <PageHeader title="Facturación" subtitle={`${facturas.length} facturas emitidas`} action={<button onClick={openNewFactura} style={{ ...btnPrimary, width: "auto", display: "flex", alignItems: "center", gap: 6 }}><Plus size={15} /> Generar factura</button>} />
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {facturas.map((f, i) => {
                ...
                return (
                  <div key={f.id} style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 9, padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      {f.estadoEnvio !== "enviada" && <button onClick={() => marcarEnviada(f)} style={btnGhost}><Send size={14} /></button>}
            ...
            <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 9, padding: 18 }}>
              ...
              {stats.topServicios.map(([nombre, count]) => (
                <div key={nombre} style={{ marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, marginBottom: 4 }}><span>{nombre}</span><span style={{ color: COLORS.textSecondary }}>{count}</span></div>
                  <div style={{ height: 6, background: COLORS.surfaceRaised, borderRadius: 4 }}><div style={{ height: 6, width: `${(count / stats.maxConteo) * 100}%`, background: COLORS.accent, borderRadius: 4 }} /></div>
                </div>
              ))}
      ...
      {showCitaModal && (
        <Modal title={editingCita ? "Editar cita" : "Nueva cita"} onClose={() => setShowCitaModal(false)}>
          ...
          <select style={inputStyle} value={citaForm.vehiculoId} onChange={(e) => setCitaForm({ ...citaForm, vehiculoId: e.target.value })}>
            <option value="">Seleccionar vehículo</option>{vehiculos.filter((v) => v.clienteId === citaForm.clienteId).map((v) => <option key={v.id} value={v.id}>{v.placa} · {v.marca} {v.modelo}</option>)}
          </select>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
      ...
      {showOrdenModal && (
        <Modal title={editingOrden ? "Editar orden de trabajo" : "Nueva orden de trabajo"} onClose={() => setShowOrdenModal(false)} wide>
          ...
          <select style={inputStyle} value={ordenForm.vehiculoId} onChange={(e) => setOrdenForm({ ...ordenForm, vehiculoId: e.target.value })}>
            <option value="">Seleccionar vehículo</option>{vehiculos.map((v) => <option key={v.id} value={v.id}>{v.placa} · {v.marca} {v.modelo}</option>)}
          </select>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
            <select style={{ ...inputStyle, marginBottom: 0, flex: 1 }} value={itemTipo} onChange={(e) => { setItemTipo(e.target.value); setItemSeleccionId(""); }}>
              <option value="servicio">Servicio</option><option value="repuesto">Repuesto</option>
            </select>
            <select style={{ ...inputStyle, marginBottom: 0, flex: 2 }} value={itemSeleccionId} onChange={(e) => setItemSeleccionId(e.target.value)}>
              <option value="">Seleccionar...</option>{(itemTipo === "servicio" ? servicios : repuestos).map((x) => <option key={x.id} value={x.id}>{x.nombre}</option>)}
            </select>
          ...
          <div style={{ marginBottom: 14 }}>
            {ordenForm.items.map((it, idx) => (
              <div key={idx} style={{ display: "flex", justifyContent: "space-between", fontSize: 13, padding: "6px 0", borderBottom: `1px solid ${COLORS.border}` }}>
                ...
                <span style={{ display: "flex", alignItems: "center", gap: 8 }}>{money(it.precio * it.cantidad)}<X size={13} style={{ cursor: "pointer", color: COLORS.danger }} onClick={() => quitarItemOrden(idx)} /></span>
              </div>
            ))}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 15, fontWeight: 600, marginBottom: 16 }}><span>Total</span><span>{money(costoOrdenActual)}</span></div>
      ...
      {showRevisionModal && (
        <Modal title="Nueva revisión" onClose={() => setShowRevisionModal(false)}>
          ...
          <select style={inputStyle} value={revisionForm.ordenId} onChange={(e) => setRevisionForm({ ...revisionForm, ordenId: e.target.value })}>
            <option value="">Seleccionar orden</option>{ordenes.map((o) => { const v = vehiculos.find((x) => x.id === o.vehiculoId); return <option key={o.id} value={o.id}>{v?.placa} · {o.fecha}</option>; })}
          </select>
      ...
      {showFacturaModal && (
        <Modal title="Generar factura" onClose={() => setShowFacturaModal(false)}>
          ...
          <select style={inputStyle} value={facturaForm.ordenId} onChange={(e) => seleccionarOrdenFactura(e.target.value)}>
            <option value="">Seleccionar orden</option>{ordenes.map((o) => { const v = vehiculos.find((x) => x.id === o.vehiculoId); return <option key={o.id} value={o.id}>{v?.placa} · {money(o.costoTotal)}</option>; })}
          </select>
